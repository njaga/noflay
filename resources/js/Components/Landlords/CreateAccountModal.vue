<template>
    <Modal :show="show" @close="handleClose">
      <div class="p-8 bg-white rounded-lg shadow-xl max-w-">
        <h3 class="text-2xl font-bold text-gray-900 mb-4 flex items-center">
          <i class="bi bi-person-plus me-2"></i>
          Créer un compte
        </h3>
        <p v-if="!accountCreated && !errorMessage" class="text-sm text-gray-600 mb-6">
          Pour : {{ landlord?.first_name }} {{ landlord?.last_name }}<br>
  ...
  const createAccount = async () => {
    ...
    try {
      const response = await axios.post(`/landlords/${props.landlord.id}/create-account`);
      accountDetails.value = response.data;
      accountCreated.value = true;
    } catch (error) {
      if (error.response && error.response.status === 400 && error.response.data.error === 'Ce bailleur a déjà un compte') {
        errorMessage.value = 'Ce bailleur a déjà un compte';
      } else {
