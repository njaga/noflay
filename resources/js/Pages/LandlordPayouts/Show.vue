<template>
    <AppLayout :title="`Versement #${payout.id}`">
        <div class="min-h-screen bg-gray-100 py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                    <div v-for="(infoSet, index) in [
                        ...
                        <div class="p-6">
                            <dl class="space-y-4">
                                <div v-for="item in infoSet" :key="item.label"
          ...
          class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-300">
    <i class="bi-receipt mr-2"></i>
    Générer un reçu
  </button>
        ...
        <!-- Iframe caché pour le téléchargement du reçu -->
...
const generalInfo = computed(() => [
    { label: "ID du versement", value: payout.id, icon: "bi-file-earmark" },
    {
        ...
        value: formatDate(payout.payout_date),
...
const landlordInfo = computed(() => [
    {
        label: "Nom",
        value: `${payout.landlord.first_name} ${payout.landlord.last_name}`,
        icon: "bi-person",
    },
...
const generateReceipt = async () => {
  try {
    const response = await axios.get(route('landlord-transactions.generate-receipt', payout.id), {
      responseType: 'blob',
    });
    ...
    link.href = url;
    link.setAttribute('download', `recu_versement_${payout.id}.pdf`);
    ...
    errorMessage.value = 'Une erreur est survenue lors de la génération du reçu.';
  }
};
