        ...
        <div class="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-8">
                    ...
                    <div v-if="canManageUsers" class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                        <Link :href="route('users.create')"
                            class="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition duration-150 ease-in-out shadow-md hover:shadow-lg w-full sm:w-auto">
                            <i class="fas fa-plus mr-2"></i>Ajouter un utilisateur
                        </Link>
                        <button @click="handleDownloadPDF"
                            class="inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition duration-150 ease-in-out shadow-md hover:shadow-lg w-full sm:w-auto">
                            <i class="fas fa-download mr-2"></i>Télécharger PDF
                        </button>
                    </div>
                </div>

                <!-- Recherche, filtres et options d'affichage -->
                <div class="mb-6 bg-white p-4 rounded-lg shadow-md">
                    <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                        ...
                        <div class="flex flex-wrap justify-between sm:justify-end space-x-2 space-y-2 sm:space-y-0">
                            <select v-model="sortBy"
                                class="rounded-lg border focus:outline-none focus:border-indigo-500 px-8 py-2">
                            ...
                            <button @click="toggleSortOrder"
                                ...
                                <i :class="['fas', sortOrder === 'asc' ? 'fa-sort-alpha-down' : 'fa-sort-alpha-up']"></i>
                            </button>
                            ...
                            <button @click="toggleView" class="hidden sm:block px-4 py-2 bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-200 transition duration-150 ease-in-out">
                ...
                <div v-else>
                    <!-- Vue grille (toujours visible sur mobile, optionnelle sur desktop) -->
                    <div v-if="isMobile || (!isMobile && viewMode === 'grid')"
                    ...
                    <!-- Vue liste (visible uniquement sur desktop) -->
                    <div v-if="!isMobile && viewMode === 'list'" class="hidden sm:block">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="user in paginatedUsers" :key="user.id" class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                        ...
                                        <td v-if="canManageUsers" class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <Link :href="route('users.show', user.id)" class="text-indigo-600 hover:text-indigo-900 mr-2">
                                                <i class="fas fa-eye"></i>
                                            </Link>
                                            <Link :href="route('users.edit', user.id)" class="text-blue-600 hover:text-blue-900 mr-2">
                                                <i class="fas fa-edit"></i>
                                            </Link>
                                            <button @click="openDeleteModal(user)" class="text-red-600 hover:text-red-900 mr-2">
...
const checkIfMobile = () => {
    isMobile.value = window.innerWidth < 640; // 640px est le breakpoint 'sm' dans Tailwind par défaut
};
