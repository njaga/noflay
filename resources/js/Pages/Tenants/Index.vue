...
const userRoles = computed(() => user?.roles?.map(role => role.name) ?? []);
    ...
    <AppLayout title="Gestion des locataires">
        <div class="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                ...
                <TenantDashboard :activeTenantCount="activeTenantCount" :inactiveTenantCount="inactiveTenantCount"
                    :expiringContracts="expiringContracts" />

                <TenantFilters :filters="filters" :landlords="landlords" @apply-filters="applyFilters"
                    @reset-filters="resetFilters" />

                <TenantTable :paginatedTenants="paginatedTenants" :canUpdateTenant="canUpdateTenant"
                    :canDeleteTenant="canDeleteTenant" @openDeleteModal="openDeleteModal" @editTenant="editTenant"
                    @viewTenant="viewTenant" @createAccount="openCreateAccountModal" />
                ...
                <TenantModal :show="showDeleteModal" @close="closeDeleteModal" :tenant="tenantToDelete"
                    @confirm="confirmDelete" />
                <CreateTenantAccountModal :show="showCreateAccountModal" @close="closeCreateAccountModal"
                    :tenant="tenantToCreateAccountFor" />
            </div>
        </div>
    </AppLayout>
</template>
